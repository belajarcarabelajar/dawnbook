...
describe("/api/auth/login", () => {
  ...
  test("stores redirect_url in oauth_redirect cookie when provided (safe path)", async () => {
    ...
    const req = new Request(
      "https://example.com/api/auth/login?redirect_url=/appreciation.html",
    );
  ...
  test("drops open-redirect attempt with //evil.com", async () => {
    ...
    setEnvHandlers(env);
    const req = new Request(
      "https://example.com/api/auth/login?redirect_url=//evil.com/x",
    );
    const res = await loginHandler({ request: req, env } as any);
    const cookies = res.headers.get("Set-Cookie")!;
  ...
  test("drops open-redirect attempt with /\\evil.com", async () => {
    ...
    setEnvHandlers(env);
    const req = new Request(
      "https://example.com/api/auth/login?redirect_url=/\\evil.com",
    );
    const res = await loginHandler({ request: req, env } as any);
    const cookies = res.headers.get("Set-Cookie")!;
    // No oauth_redirect cookie should be set when redirect_url is unsafe.
...
describe("/api/auth/callback", () => {
  ...
  test("rejects state mismatch", async () => {
    ...
    const req = new Request(url, {
      headers: { Cookie: `oauth_state=00ff00ff` },
    });
  ...
  test("falls back to / when redirect_url cookie missing or invalid", async () => {
    ...
    const req = new Request(url, {
      headers: { Cookie: `oauth_state=${STATE_HEX}` },
    });
    const res = await callbackHandler({ request: req, env } as any);
    ...
    setQueryHandler(env, "SELECT", () => [
      {
        s_id: SESSION_ID_HEX,
        s_user_id: "u",
        s_expires_at: "2000-01-01T00:00:00.000Z",
        s_created_at: "2000-01-01T00:00:00.000Z",
        s_last_seen_at: "2000-01-01T00:00:00.000Z",
        s_user_agent: null,
        s_ip: null,
        u_id: "u",
        u_google_sub: "g",
        ...
        u_created_at: "2000-01-01T00:00:00.000Z",
        u_last_login_at: "2000-01-01T00:00:00.000Z",
      },
    ]);
    const req = new Request("https://example.com/api/auth/me", {
      headers: { Cookie: `session_id=${SESSION_ID_HEX}` },
