...
**R14 — Mandatory Manual Line-by-Line File Reading and Inspection Rule (Zero Blind Execution Policy)**

*Statement:*
EVERY AI agent or subagent MUST manually read EVERY SINGLE file line-by-line using the `view_file` tool before making any edits, fixing errors, or declaring Phase B/Phase D complete. Relying solely on automated regular expression scripts (e.g. `sed`, mass `replace`, or batch node scripts) without reading the files line-by-line is STRICTLY FORBIDDEN. Automated pre-flight check scripts (`check-latex-support.ts` & `check-media-support.ts`) are verification tools ONLY, NOT a replacement for manual file inspection.

*Step-by-Step Mandatory Execution Workflow for Any AI Agent:*

1. **Step 1 — Mandatory Full File View:**
   - Call `view_file` on EVERY chapter file (`01_...md`, `02_...md`, up to `13_...md`) from line 1 to the end of the file.
   - You MUST inspect the text line-by-line with your own context window. DO NOT skip viewing any file.

2. **Step 2 — Line-by-Line LaTeX & Content Checklist:**
   For every line viewed, verify:
   - **Inline Math Delimiters:** Must strictly use `\( ... \)`. Never use unescaped single `$` or raw LaTeX.
   - **Block Math Delimiters:** Must strictly be formatted as single-line `\[ ... \]` on one single line in the Markdown file. NEVER split `\[` and `\]` across multiple lines (which causes mdBook's pulldown-cmark parser to break the block into separate `<p>` tags, ruining MathJax rendering). NEVER double-wrap like `$$\\[ ... \\]$$` or `$$\(` or `$\(`.
   - **Multi-Letter Variables:** Multi-letter economic/scientific variables (e.g. `TR`, `AR`, `MR`, `ATC`, `MC`, `DWL`, `HHI`, `WTP`, `FC`, `TC`, `AC`) inside math blocks MUST be wrapped in `\text{...}` (e.g., `\text{MR}`, `\text{MC}`).
   - **Formula Context Accuracy:** Check that every formula matches the surrounding narrative paragraph. Ensure no copy-pasted supply-demand equations (`Q_d = a - bP`) appear in chapters dealing with Game Theory, Elasticity, or Cost Functions.
   - **No Garbled Text:** Ensure no raw math formulas are merged directly into narrative prose sentences or wrapped in invalid syntax like `$\( ... \)$`.
   - **No Em-Dash (`—`):** Ensure zero em-dashes exist in the text. Replace with commas, parentheses, colons, or regular hyphens.

3. **Step 3 — Surgical Targeted Edits:**
   - If any formatting error is found during manual reading, use `replace_file_content` targeting that specific line range.
   - DO NOT run blind mass-replacement regex scripts across the entire directory.

4. **Step 4 — Post-Check Verification:**
   - After manual inspection and surgical editing, execute `bun run scripts/check-latex-support.ts` and `bun run scripts/check-media-support.ts`.
   - If a script flags a line, DO NOT attempt automated mass fixes. Open the flagged file using `view_file`, inspect lines around the error, understand the exact context, and fix that line manually with `replace_file_content`.

*Acceptance check:* `view_file` was invoked for all chapter files in `src/content/`. `check-latex-support.ts` and `check-media-support.ts` both exit with code 0.
...
**R23 — Mandatory Web Search Verification for Reference Citations & DOIs**

*Statement:*
Every citation entry in a book's dedicated `Referensi` chapter MUST be 100% authentic, relevant, and verified via `search_web`. Subagents and agents are STRICTLY FORBIDDEN from guessing, hallucinating, or fabricating reference titles, author names, DOIs, or generic homepage URLs.
When authoring or editing the `Referensi` chapter:
1. Subagents MUST execute `search_web` for EVERY single cited item to confirm the exact title, authors, publication year, volume/issue, and exact DOI or deep-link URL.
2. Every reference entry MUST explicitly include a clickable markdown hyperlink `[Judul/Nama Karya](URL)` or direct DOI resolution link `https://doi.org/10.xxxx/xxxx` targeting the exact publication landing page (e.g. official government law portals `peraturan.bpk.go.id`, Eur-Lex `eur-lex.europa.eu`, `https://doi.org/...`, UNESCO Digital Library, Leiden Digital Collections, Internet Archive, JSTOR, or publisher book page). Generic root domain homepages (e.g. `https://brill.com/` or `https://jstor.org/`) are STRICTLY FORBIDDEN.
3. Plain-text citations without markdown hyperlinks are STRICTLY FORBIDDEN.

*Acceptance check:* Every reference entry in `src/content/*_referensi.md` is verified via `search_web` and contains an exact, direct, non-generic markdown hyperlink `[...](https://...)`.
...
**S7 — Clerk Authentication Isolation Rule**

*Surface:* `functions/_middleware.ts`, `functions/lib/auth.ts`, `functions/lib/gating.ts`, and all edge-level authentication configurations.

*Enforced constraint:* During the creation or modification of books, agents MUST NOT modify, rewrite, or attempt to troubleshoot any Clerk authentication logic, JWT verification middleware, gating redirects, or security keys. The authentication system is isolated and locked. Modifying any auth code during content processing is strictly forbidden.

*Required end state:* The edge and client-side Clerk integration remains completely untouched and preserved in its verified stable state during all book additions.
...
- **Rule 7.7.2 — LaTeX Rule:** Raw LaTeX MUST be converted to `\\( ... \\)` for inline and `\\[ ... \\]` (or `$$ ... $$`) for block math. You MUST use double backslashes so markdown parsers don't swallow them. If a word or multiple letters appear inside a math block (e.g., as a variable like `Ego` or `Efisiensi`), it MUST be wrapped in `\text{...}` (e.g., `\text{Ego}`). KaTeX strict mode will fail on unformatted multi-letter variables. **CRITICAL FOR MOBILE:** If a block math equation (`\\[ ... \\]`) is wide or contains multiple terms, you MUST use a multi-line environment like `\begin{aligned} ... \end{aligned}` to prevent horizontal scrolling issues on mobile devices. *(Cross-ref: R10)*
...
- **Rule 7.7.5 — Dedicated Reference Chapter (Mandatory Web Search & Deep Links):** A dedicated chapter specifically for "Referensi" MUST be generated and appended at the end of the book. The subagent responsible for this chapter MUST execute `search_web` for EVERY single cited item to find robust, trustworthy literature and verified DOIs corresponding to the book's topic. **Strict Constraints:**
  1. The sources MUST be restricted EXCLUSIVELY to scientific journal articles, books, and official government regulations.
  2. Subagents are STRICTLY FORBIDDEN from guessing, hallucinating, or fabricating titles, authors, DOIs, or generic homepage URLs.
  3. Every reference entry MUST contain a working, exact deep-link markdown hyperlink `[Judul/Nama Karya](URL)` or DOI link `https://doi.org/...` targeting the exact publication landing page. Generic root domain homepages (e.g., `https://brill.com/` or `https://jstor.org/`) are STRICTLY FORBIDDEN.
  4. Plain-text reference entries without markdown hyperlinks are STRICTLY FORBIDDEN. All references MUST be consistently formatted in APA Style 7th Edition. (This rule applies only to new books processed going forward; existing books are exempt).
- **Rule 7.7.6 — Single-Pass Write:** When processing the chapter file, the subagent MUST execute the write operation in a single pass using the `write_to_file` tool with `Overwrite: true` (or outputting the entire file at once). Do NOT use line-by-line iterative edit tools (`replace_file_content`) to prevent the agent from getting stuck.
- **Rule 7.7.7 — Em-Dash Rule:** The character em-dash (`—`) is strictly forbidden in the final book output. Subagents must remove it entirely, replacing it with a comma (`,`), parentheses `( )` for parenthetical explanations, a colon (`:`) for definition clauses, or a regular hyphen (`-`) for quotation attributions, or rephrasing the sentence.
- **Rule 7.7.8 — Rich Markdown and Layout Rule:** Every chapter markdown file MUST be visually rich, beautiful, and structured to maximize readability. Subagents MUST actively employ standard markdown formatting features, including:
  - **Tables** (`| Column 1 | Column 2 |`) for comparison data, contrast matrices, or structured summaries.
  - **Blockquotes** (`>`) for all direct quotes from the text, books, or definitions, setting them apart elegantly.
  - **Structured Lists** (bulleted or numbered lists) to present classifications, multi-item details, or logical steps.
...
[ ] Phase B: All chapter files must not contain the em-dash (—) character.
...
- **SEO Rules & Criteria**: [docs/SEO_Rules_and_Criteria.md](docs/SEO_Rules_and_Criteria.md) — Comprehensive guidelines and knowledge base for AI agents on technical and on-page SEO.
- **CORS Audit Report**: [docs/CORS_AUDIT_REPORT.md](docs/CORS_AUDIT_REPORT.md) — Analysis of Cross-Origin Resource Sharing policies.
- **Secret Audit Report**: [docs/SECRET_AUDIT_REPORT.md](docs/SECRET_AUDIT_REPORT.md) — Audit of exposed secrets and vulnerabilities.
- **Reorg Plan**: [docs/REORG_PLAN.md](docs/REORG_PLAN.md) — Structural reorganization planning document.
