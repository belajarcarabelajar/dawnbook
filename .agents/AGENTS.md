...
- **Mandatory Manual Line-by-Line Inspection Rule (Rule R14)**: Agents and subagents MUST call `view_file` to manually read EVERY SINGLE chapter file line-by-line BEFORE making edits or running verification tools. Using automation scripts, Node batch converters, or mass search-and-replace scripts for LaTeX tasks is STRICTLY FORBIDDEN. All LaTeX formatting, fixes, and verification MUST be performed directly by the AI agent using generative AI tools (`view_file` and `replace_file_content`). Check all inline math `$ ... $`, display math `$$ ... $$`, list item indentations, multi-letter `\text{...}` variables, and context accuracy manually. Automated check scripts are ONLY post-flight verification tools, never a replacement for manual file reading.
