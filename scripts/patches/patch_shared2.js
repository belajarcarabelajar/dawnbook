...
code = code.replace(/} else \{\s+window\.checkpointHandled = true;(?:\s+window\.saveProgress\(\);)?\s+\}\s+\}\)\s+\.catch\(function\(e\) \{\s+console\.error\('Failed to load progress', e\);\s+window\.checkpointHandled = true;(?:\s+window\.saveProgress\(\);)?\s+\}\);/, `} else {
