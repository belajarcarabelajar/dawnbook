...
async function checkMediaSupport() {
  ...
  const sharedCssPath = join(booksDir, "shared-header-v4.css");
  ...
  if (!cssContent.includes(".embed-responsive")) {
    console.error(`❌ [FAIL] shared-header-v4.css is missing '.embed-responsive' class`);
    hasErrors = true;
  }
  if (!cssContent.includes("img {") || !cssContent.includes("max-width: 100%")) {
    console.error(`❌ [FAIL] shared-header-v4.css is missing responsive 'img' constraints`);
    hasErrors = true;
  }
